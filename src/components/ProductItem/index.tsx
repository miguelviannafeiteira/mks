...
export function ProductItem({ addToCart, name, price, url, description }: Props) {
    return (
        <div data-testid="product" className="w-[220px] h-[360px] shadow-lg rounded-xl flex flex-col justify-between relative">
            <div className="px-4 pt-2">
                <Image src={url} alt={description} width={210} height={138} />
            </div>

            <div className="">
                <div className="flex gap-[8px] px-4 pt-2 items-start justify-between h-[70px]">
                    <h3 className="leading-5 font-normal text-base text-[#2C2C2C]">
                        {name}
                    </h3>

                    <span className="bg-[#373737] text-white font-bold py-1 px-2 rounded-lg">
                        R${formatPrice(price)}
                    </span>
                </div>
